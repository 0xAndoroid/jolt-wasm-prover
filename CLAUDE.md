...
- `src/gpu/stage2.rs` — `WebGpuRelationRange`, the `akita_prover::RelationRangeDevice` for the browser (features `webgpu,relation-range-device`, W3): takes every stage-2 level whose relation domain is ≥ 2^16 — quotient-factored levels (alpha × lane weights + packing/sparse linear segments, host folds alpha and the sources per coefficient round) and reduced-dense levels (flat weight table with the linear terms folded in) — runs `log2(domain) − 12` rounds (one RUN_SEQ per round: fused fold + evaluate pass, sparse additional-terms pass, reduce; 160 B inline readback; the last round also copies the folded `[P | W]` tables) and hands the CPU the 2^12 tail. Parity: `set_relation_range_parity_rounds` keeps the CPU authoritative and compares round polynomials + handoff tables (`stage2_device_parity_ok` / `stage2_device_tables_ok` trace events). Kernels `frontend/public/wgsl/stage2/`, oracle `bench/test_stage2_wgsl.py`
...
- **`relation-range-device`** — `engine::install_relation_range_device` + `CpuReferenceDevice` for the stage-2 relation-range sumcheck (patch 0008; `JOLT_RELATION_RANGE_DEVICE=cpu-ref` natively); with `webgpu` also `gpu::stage2::WebGpuRelationRange` (W3). `webgpu,trace-commit-device,digit-range-device,relation-range-device` is the full browser build
