...
async function run() {
    ...
    process.stderr.write('[bench-debug] browser ' + browser.version() + '\n');
    page.on('worker', (w) => {
        try { w.on('console', (msg) => process.stderr.write('[worker-console] ' + msg.text() + '\n')); } catch {}
    });
    ...
    await page.evaluate(async ({ tracing, webgpu }) => {
        try {
            ...
            chan.onmessage = (m) => console.log('[wedge-channel] ' + JSON.stringify(m.data));
        } catch {}
