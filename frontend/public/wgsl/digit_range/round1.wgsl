...
// Workgroup = one segment of blk = min(inner, 2048) groups sharing e_out; shared 256-class
...
@group(0) @binding(4) var<storage, read> lut1: array<vec4<u32>>; // 256 x 5
...
fn main(@builtin(local_invocation_id) l: vec3<u32>, @builtin(workgroup_id) w: vec3<u32>) {
  ...
  let base = wg * blk;
  let end = min(base + blk, params.n_units);
  for (var g: u32 = base + lid; g < end; g += WG) {
  ...
  if (base < params.n_units) {
    let eo = eq[params.off_second + (base >> params.inner_bits)];
    for (var c: u32 = 0u; c < 5u; c++) { acc[c] = fp128_mul(fp128_mul(hv, lut1[5u * lid + c]), eo); }
  } else {
