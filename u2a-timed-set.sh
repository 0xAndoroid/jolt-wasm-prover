...
# Usage: PORT=8143 ./u2a-timed-set.sh
set -u
cd "$(dirname "$0")"
PORT="${PORT:-8153}"
...
BENCH_BASE="$BASE" BENCH_WEBGPU='{}' PW_CHANNEL=chrome \
    perl -e 'alarm 700; exec @ARGV' -- node trace-chain.mjs 1112 1 traces-u2a/on 2>/dev/null | tail -1
BENCH_BASE="$BASE" PW_CHANNEL=chrome \
    perl -e 'alarm 700; exec @ARGV' -- node trace-chain.mjs 1112 1 traces-u2a/off 2>/dev/null | tail -1

echo "== soak 5 @2^22 =="
gpu
PW_CHANNEL=chrome perl -e 'alarm 2400; exec @ARGV' -- \
    node soak-webgpu.mjs 1112 8 4 "$BASE" traces-u2a/u2a-soak.jsonl 2>&1 | tail -4
